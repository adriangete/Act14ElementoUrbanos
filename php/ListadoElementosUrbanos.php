...
$pdf->Cell(14,$alturafila2,utf8_decode('Direccion'),1,0,'L',True);
$pdf->Cell(20,$alturafila2,utf8_decode('Fecha.'),1,0,'C');
$pdf->Cell(20,$alturafila2,utf8_decode('Hora'),1,1,'L',True);
...
if ($connect->connect_errno) {
    ...
    if (!$resultado) {
        ...
        while ($fila = mysqli_fetch_row($resultado)) {
			...
			$pdf->Cell($w[0],$alturafila2,utf8_decode($fila[2]),1,0,'L',$pintaFondo);
			$pdf->Cell($w[1],$alturafila2,utf8_decode($fila[9]),1,0,'L',$pintaFondo);
			$pdf->SetFont('Arial','B',10);
			$pdf->Cell($w[2],$alturafila2,utf8_decode($fila[8]),1,0,'L',$pintaFondo);
			$pdf->SetFont('Arial','',10);
			$pdf->Cell($w[3],$alturafila2,utf8_decode($fila[5]),1,0,'C',$pintaFondo);
			$pdf->Cell($w[4],$alturafila2,utf8_decode($fila[4]),1,0,'R',$pintaFondo);
			$pdf->Ln();
...
$pdf->Ln();
//$pdf->Output();  
$filename="ElementosUrbanos.pdf";
//Output sirve para descargar
